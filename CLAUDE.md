...
**Multi-provider LLM**: `LlmProvider` entity z presetami per typ (Ollama/LMStudio/vLLM/OpenAI/Azure/Groq/Together/Mistral/DeepSeek/LocalAI). `IChatClientFactory.GetForAsync(providerId)` — cache HttpClient per providerId. `VllmCheckConfig.LlmProviderId?` pozwala triggerowi wybrać swój provider (default → IsDefault flag).
