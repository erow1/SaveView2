...
- **ONNX Runtime** (ML inference — YOLO v5/v8/v9 closed-set, YOLO-World v2-x **Apache 2.0** open-vocab text-prompt, **OWLv2 base patch16 ensemble Apache 2.0** open-vocab text-prompt z lepszym pokryciem rzadkich klas, YOLOE AGPL text+visual swap-ready, CLIP ViT-B/32 text+image encoders)
...
33. **OWLv2 jako alternatywa do YOLO-World** (2026-04-27) — YOLO-World v2-x ma słabe pokrycie rzadkich klas i części obiektów (face, pants, shoe, hand). Dla tych przypadków dodany **OWLv2** (Google, Apache 2.0): `runtime/models/owlv2-base/`, single fused ONNX, 960×960 input, 3600 anchors, 153M params. Implementacja w `src/SafeView.ML/OwlV2/OnnxOwlV2Detector.cs`, `DetectorBackend.OwlV2 = 4`. ModelSeeder rozpoznaje folder po `preprocessor_config.json` + `tokenizer/`, ustawia `Capabilities = TextPrompts`, `InputSize = 960`. User przełącza model w `/models` test detect dropdown — tym samym pipeline-em (`ITextPromptDetector` factory routing). Tokenizer (CLIP BPE) wspólny z YW. OWLv2 jest ~3-5× wolniejszy CPU ale lepiej radzi sobie z partami ciała / ubrania. Polecane do scenariuszy gdzie YW v2-x daje słabe wyniki dla nietypowych klas.
